...
- 取得日数: 7日間
- 取得項目: `最高/最低気温`, `天気コード`, `最大風速`, `相対湿度`, `降水確率`, `時間別の降水・天気`
- キャッシュ: 地点ごとに1時間 `localStorage` キャッシュ（1時間以内はAPI未呼び出し）
- 判定ロジック:
	- 日中(6:00-18:00)の雨あり または 日中降水確率60%以上 → `洗濯非推奨（White）`
	- 日中の雨なし かつ 夜間(18:00-翌6:00)のみ雨 → `外干しOK（早朝/夜の雨に注意）`
	- `湿度65%未満` かつ `晴れ寄り` かつ `日中降水確率20%未満` かつ `夜間の雨なし` → `絶好の洗濯日和（Gold）`
	- それ以外 → 中間判定（外干しOK、必要に応じて風/雨/湿気の注意を表示）
