...
- 取得日数: 7日間
- 取得項目: `最高/最低気温`, `天気コード`, `最大風速`, `相対湿度`, `降水確率`
- キャッシュ: 地点ごとに1時間 `localStorage` キャッシュ（1時間以内はAPI未呼び出し）
- 判定ロジック:
	- `湿度65%未満` かつ `晴れ` → `絶好の洗濯日和（Gold）`
	- `降水確率60%以上` または `雨` → `洗濯非推奨（White）`
	- `風速強め` かつ `降水確率30%以上` → `洗濯非推奨（風・雨に注意）`
	- `降水確率30%以上` のみ → `洗濯非推奨（White）`
	- `風速強め` のみ → 中間判定（外干しOK・風に注意）
	- それ以外 → 中間判定（外干しOK）
